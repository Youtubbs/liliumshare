#!/usr/bin/env python3
import os, sys, json, pathlib, subprocess, signal, base64
import tkinter as tk
from tkinter import ttk, messagebox, filedialog
from pathlib import Path
import requests

# ---- optional desktop notifications (best effort) ----
...
def _keys_file(keys_home: pathlib.Path) -> pathlib.Path:
    return keys_home / ".liliumshare" / KEYS_NAME

def load_keys(keys_home: pathlib.Path):
    kf = _keys_file(keys_home)
    if not kf.exists():
        raise FileNotFoundError(f"Missing keys at {kf}")
    return json.loads(kf.read_text())

def short(s: str, n=10):
    return s[:n] + "…" if s and len(s) > n else s

# ---- in-process key generation & registration ----
def _b64(b: bytes) -> str:
    return base64.b64encode(b).decode("ascii")

def generate_keys_in_dir(target_home: pathlib.Path, nickname: str | None, backend_http: str) -> dict:
    """
    Generates keys at <target_home>/.liliumshare/keys.json and registers with backend.
    Returns the dict.
    """
    from cryptography.hazmat.primitives.asymmetric import rsa
    from cryptography.hazmat.primitives import serialization

    cfg_dir = target_home / ".liliumshare"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    kf = cfg_dir / KEYS_NAME

    # create or overwrite (explicit flow is clearer than silent reuse)
    sk = rsa.generate_private_key(public_exponent=65537, key_size=2048)
    pub_der = sk.public_key().public_bytes(
        serialization.Encoding.DER,
        serialization.PublicFormat.SubjectPublicKeyInfo
    )
    priv_der = sk.private_bytes(
        serialization.Encoding.DER,
        serialization.PrivateFormat.PKCS8,
        serialization.NoEncryption()
    )
    data = {"private": _b64(priv_der), "public": _b64(pub_der), "nickname": nickname or None}
    kf.write_text(json.dumps(data, indent=2))

    # register
    try:
        r = requests.post(backend_http.rstrip("/") + "/api/register",
                          json={"pubkey": data["public"], "nickname": nickname or None}, timeout=8)
        r.raise_for_status()
    except Exception as e:
        # keep local keys; surface error
        raise RuntimeError(f"Keys generated, but registration failed: {e}")
...
class App(tk.Tk):
    def __init__(self):
        ...
        # child processes / windows we toggle
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Health", command=self._health_check).grid(row=0, column=5, padx=(18,2))

        ttk.Label(top, text="Keys Home:").grid(row=0, column=6, sticky="e", padx=(18,2))
        ttk.Entry(top, textvariable=self.keys_home, width=26).grid(row=0, column=7, sticky="we")
        ttk.Button(top, text="Browse…", command=self._pick_home).grid(row=0, column=8, padx=4)
        ttk.Button(top, text="Load Keys", command=self._load_my_keys).grid(row=0, column=9, padx=4)
        ttk.Button(top, text="Generate keys…", command=self._open_keygen).grid(row=0, column=10, padx=4)
        ...
        ttk.Button(addf, text="Send Friend Request", command=self._send_request)\
            .grid(row=0, column=2, rowspan=2, sticky="nsw", padx=6, pady=4)
    ...
    def _health_check(self):
        try:
            r = requests.get(self.base.get().rstrip("/") + "/health", timeout=10)
            r.raise_for_status()
    ...
    def _open_keygen(self):
        w = tk.Toplevel(self); w.title("Generate keys")
        w.transient(self); w.grab_set()
        w.resizable(False, False)

        dir_var = tk.StringVar(value=self.keys_home.get())
        nick_var = tk.StringVar(value=os.getenv("USER") or os.getenv("USERNAME") or "")
        backend_var = tk.StringVar(value=self.base.get())

        frm = ttk.Frame(w); frm.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

        ttk.Label(frm, text="Keys directory (your HOME for keys):").grid(row=0, column=0, sticky="w")
        e_dir = ttk.Entry(frm, textvariable=dir_var, width=44); e_dir.grid(row=1, column=0, sticky="we", pady=(0,6))
        def _pick():
            p = filedialog.askdirectory(initialdir=dir_var.get() or str(pathlib.Path.home()))
            if p: dir_var.set(p)
        ttk.Button(frm, text="Browse…", command=_pick).grid(row=1, column=1, padx=6)

        ttk.Label(frm, text="Nickname (optional):").grid(row=2, column=0, sticky="w")
        ttk.Entry(frm, textvariable=nick_var, width=32).grid(row=3, column=0, sticky="w", pady=(0,6))

        ttk.Label(frm, text="Backend URL:").grid(row=4, column=0, sticky="w")
        ttk.Entry(frm, textvariable=backend_var, width=44).grid(row=5, column=0, sticky="we", pady=(0,6))

        status = tk.StringVar(value="")
        ttk.Label(frm, textvariable=status, foreground="#555").grid(row=6, column=0, columnspan=2, sticky="w")

        btns = ttk.Frame(frm); btns.grid(row=7, column=0, columnspan=2, sticky="e", pady=(8,0))
        def _go():
            home = pathlib.Path(dir_var.get().strip()).expanduser()
            if not str(home):
                messagebox.showerror("Keys", "Choose a directory."); return
            home.mkdir(parents=True, exist_ok=True)
            try:
                data = generate_keys_in_dir(home, nick_var.get().strip() or None, backend_var.get().strip())
                self.keys_home.set(str(home))
                self.me_pub.set(data["public"])
                self.me_nick.set(data.get("nickname") or "")
                status.set("Success — keys saved and registered.")
                messagebox.showinfo("Generate keys", "Keys generated and registered successfully.")
                w.destroy()
                self._reload_lists()
            except Exception as e:
                status.set(str(e))
                messagebox.showerror("Generate keys", str(e))
        ttk.Button(btns, text="Generate & Register", command=_go).pack(side="right", padx=6)
        ttk.Button(btns, text="Cancel", command=w.destroy).pack(side="right")
    ...
    def _start_poll(self):
        self.after(900, self._reload_lists)
        self._blink_tick()
    ...
    def _toggle_message(self):
        ...
        self._refresh_proc_styles()
        # ping a small notification to confirm open
        _notify("LiliumShare", f"Chat opened with {short(peer, 16)}")
